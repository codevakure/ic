...
describe('OpenID Connect Federated Provider Token Integration', () => {
  ...
  describe('processOpenIDPlaceholders', () => {
    ...
    it('should replace specific OpenID Connect placeholders', () => {
      const template = `
        Access: {{RANGER_OPENID_ACCESS_TOKEN}}
        ID: {{RANGER_OPENID_ID_TOKEN}}
        User: {{RANGER_OPENID_USER_ID}}
        Email: {{RANGER_OPENID_USER_EMAIL}}
        Name: {{RANGER_OPENID_USER_NAME}}
    ...
    it('should handle missing token fields gracefully', () => {
      ...
      const template = 'Token: {{RANGER_OPENID_TOKEN}}, Email: {{RANGER_OPENID_USER_EMAIL}}';
  ...
  describe('Integration with resolveHeaders', () => {
    it('should resolve OpenID Connect placeholders in headers for Cognito', () => {
      const headers = {
        'Authorization': '{{RANGER_OPENID_TOKEN}}',
        'X-User-ID': '{{RANGER_OPENID_USER_ID}}',
        'X-User-Email': '{{RANGER_OPENID_USER_EMAIL}}',
      };
    ...
    it('should work with specific access token placeholder', () => {
      const headers = {
        'Authorization': 'Bearer {{RANGER_OPENID_ACCESS_TOKEN}}',
        'X-Cognito-ID-Token': '{{RANGER_OPENID_ID_TOKEN}}',
      };
  ...
  describe('Integration with processMCPEnv', () => {
    it('should process OpenID Connect placeholders in MCP environment variables for Cognito', () => {
      const mcpOptions = {
        ...
        args: ['server.js'],
        env: {
          'COGNITO_ACCESS_TOKEN': '{{RANGER_OPENID_TOKEN}}',
          'USER_ID': '{{RANGER_OPENID_USER_ID}}',
          'USER_EMAIL': '{{RANGER_OPENID_USER_EMAIL}}',
        },
      };
    ...
    it('should process OpenID Connect placeholders in MCP headers for HTTP transport', () => {
      const mcpOptions = {
        ...
        headers: {
          'Authorization': 'Bearer {{RANGER_OPENID_ACCESS_TOKEN}}',
          'X-Cognito-User-Info': '{{RANGER_OPENID_USER_EMAIL}}',
          'X-Cognito-ID-Token': '{{RANGER_OPENID_ID_TOKEN}}',
        },
      };
    ...
    it('should handle AWS-specific MCP server configuration', () => {
      const awsMcpOptions = {
        ...
        env: {
          'AWS_COGNITO_TOKEN': '{{RANGER_OPENID_ACCESS_TOKEN}}',
          'AWS_COGNITO_ID_TOKEN': '{{RANGER_OPENID_ID_TOKEN}}',
          'COGNITO_USER_SUB': '{{RANGER_OPENID_USER_ID}}',
        },
      };
  ...
  describe('Security and Edge Cases', () => {
    it('should not process OpenID Connect placeholders for expired tokens', () => {
      ...
      expect(resolvedHeaders['Authorization']).toBe('Bearer {{RANGER_OPENID_TOKEN}}');
    });
