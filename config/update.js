...
(async () => {
  ...
  if (docker) {
    ...
    console.purple('Pruning all Ranger Docker images...');

    const imageName = singleCompose ? 'ranger_single' : 'ranger';
    try {
      execSync(`${sudo}docker rmi ${imageName}:latest`, { stdio: 'inherit' });
    } catch (e) {
      console.purple('Failed to remove Docker image ranger:latest. It might not exist.');
    }
    console.purple('Removing all unused dangling Docker images...');
    execSync(`${sudo}docker image prune -f`, { stdio: 'inherit' });
    console.purple('Building new Ranger image...');
  ...
  console.green('Your Ranger app is now up to date! Start the app with the following command:');
  console.purple(startCommand);
  console.orange(
    "Note: it's also recommended to clear your browser cookies and localStorage for Ranger to assure a fully clean installation.",
  );
