...
const { getBalanceConfig } = require('@ranger/api');
const { User, Balance } = require('@ranger/data-schemas').createModels(mongoose);
...
(async () => {
  ...
  if (!balanceConfig?.enabled) {
    console.red(
      'Error: Balance is not enabled. Use ranger.yaml to enable it',
    );
    silentExit(1);
  }
