...
import { useOnClickOutside, useMediaQuery } from '@ranger/client';
import { useGetCustomConfigSpeechQuery } from 'ranger-data-provider/react-query';
import {
      ...
      <div className="sticky -top-1 z-50 mb-4 bg-white dark:bg-gray-700">
        <Tabs.List className="flex justify-center bg-background">
          <Tabs.Trigger
            ...
            className={cn(
              'group m-1 flex select-none items-center justify-center gap-2 bg-transparent px-4 py-2 text-sm text-text-secondary transition-all duration-200 ease-in-out radix-state-active:bg-secondary radix-state-active:text-foreground radix-state-active:shadow-lg',
