...
import { DropdownPopup, TooltipAnchor, Spinner, useToastContext } from '@ranger/client';
import type { TConversationTag } from 'ranger-data-provider';
...
const BookmarkMenu: FC = () => {
  ...
  const renderButtonContent = () => {
    ...
    if ((tags?.length ?? 0) > 0) {
      return <BookmarkFilledIcon className="h-4 w-4" aria-label="Filled Bookmark" />;
    }
    return <BookmarkIcon className="h-4 w-4" aria-label="Bookmark" />;
  };
          ...
          <TooltipAnchor
            ...
            render={
              <Ariakit.MenuButton
                ...
                className={cn(
                  'mt-text-sm flex h-8 w-8 flex-shrink-0 items-center justify-center gap-2 rounded-lg border border-border-light text-sm transition-colors duration-200 hover:bg-surface-hover',
