...
jest.mock('@ranger/api', () => ({
  ...
  getCustomEndpointConfig: jest.fn().mockReturnValue({
    ...
    headers: { 'x-user': '{{RANGER_USER_ID}}', 'x-email': '{{RANGER_USER_EMAIL}}' },
...
describe('custom/initializeClient', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    const { getCustomEndpointConfig, resolveHeaders, getOpenAIConfig } = require('@ranger/api');
  ...
  it('throws if endpoint config is missing', async () => {
    const { getCustomEndpointConfig } = require('@ranger/api');
