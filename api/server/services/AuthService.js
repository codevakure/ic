...
const { isEnabled, checkEmailConfig, isEmailDomainAllowed } = require('@ranger/api');
const { ErrorTypes, SystemRoles, errorsToString } = require('ranger-data-provider');
const {
  findUser,
  findToken,
  createUser,
...
const sendVerificationEmail = async (user) => {
  ...
  await sendEmail({
    ...
    payload: {
      appName: process.env.APP_TITLE || 'Ranger',
...
const setAuthTokens = async (userId, res, _session = null) => {
  try {
    ...
    res.cookie('token_provider', 'ranger', {
      expires: new Date(refreshTokenExpires),
      httpOnly: true,
      secure: isProduction,
      sameSite: 'strict',
