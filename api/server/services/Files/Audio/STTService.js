...
const { genAzureEndpoint } = require('@ranger/api');
const { extractEnvVariable, STTProviders } = require('ranger-data-provider');
...
class STTService {
  ...
  async getProviderSchema(req) {
    ...
    if (!sttSchema) {
      throw new Error(
        'No STT schema is set. Did you configure STT in the custom config (ranger.yaml)?',
      );
    }
