...
const { logAxiosError, resolveHeaders } = require('@ranger/api');
const { EModelEndpoint, defaultModels } = require('ranger-data-provider');
...
jest.mock('@ranger/api', () => {
  const originalUtils = jest.requireActual('@ranger/api');
  return {
    ...originalUtils,
...
describe('fetchModels with createTokenConfig true', () => {
  ...
  beforeEach(() => {
    ...
    const _utils = require('@ranger/api');
  ...
  it('creates and stores token configuration if createTokenConfig is true', async () => {
    ...
    const { processModelData } = require('@ranger/api');
...
describe('getOpenAIModels with mocked config', () => {
  it('uses alternative behavior when userProvidedOpenAI is true', async () => {
    ...
    jest.mock('ranger-data-provider', () => {
      const original = jest.requireActual('ranger-data-provider');
      return {
        ...original,
        defaultModels: {
