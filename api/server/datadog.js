 ...
 * 
 * Initializes the Datadog tracer using the @librechat/datadog-llm-observability package.
 * This file must be required before any other modules to ensure proper instrumentation.
 */

const { initializeDatadog } = require('@librechat/datadog-llm-observability');
...
if (!isDatadogEnabled) {
  ...
  try {
    ...
    const tracer = initializeDatadog({
      ...
      mlApp: process.env.DD_LLMOBS_ML_APP || 'librechat',
