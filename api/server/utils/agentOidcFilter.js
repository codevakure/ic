 ...
 * 
 * BUSINESS LOGIC:
 * - RangerAdmin group: Can see all agents (no filtering applied)
 * - RangerConsumer group: Can only see "Consumer Agent" (filtered by title)
 * - RangerEconomic group: Can only see "Economic" and "Marketplace" agents (filtered by title)
...
function getPermittedAgentTitles(oidcGroups) {
  ...
  // RangerAdmin group - can see all agents (no filtering)
  if (oidcGroups.includes('RangerAdmin')) {
    logger.debug('[agentOidcFilter] User has RangerAdmin group - allowing access to all agents');
  ...
  // RangerConsumer group - Consumer Agent
  if (oidcGroups.includes('RangerConsumer')) {
    logger.debug('[agentOidcFilter] User has RangerConsumer group - adding Consumer Agent access');
    permittedTitles.push('Consumer Agent'); // Exact match for agent with "Consumer Agent" in title
  }

  // RangerEconomic group - Economic and Marketplace agents
  if (oidcGroups.includes('RangerEconomic')) {
    logger.debug('[agentOidcFilter] User has RangerEconomic group - adding Economic and Marketplace agents access');
 ...
 * - Users with RangerAdmin: See all agents (admin access)
 * - Users with RangerConsumer: Only see agents with "Consumer Agent" in title
 * - Users with RangerEconomic: Only see agents with "Economic" or "Marketplace" in title
