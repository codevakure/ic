...
const { EModelEndpoint, Constants, openAISettings, CacheKeys } = require('ranger-data-provider');
...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing Ranger conversation');
    return importRangerConvo;
  }
...
/**
 * Imports a Ranger conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importRangerConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
) {
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid Ranger file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from Ranger file`, error);
  }
}
