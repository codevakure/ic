import { logger, webSearchKeys } from '@ranger/data-schemas';
import { Constants, extractVariableName } from 'ranger-data-provider';
import type { TCustomConfig } from 'ranger-data-provider';
import type { AppConfig } from '@ranger/data-schemas';
...
const deprecatedVariables = [
  {
    key: 'CHECK_BALANCE',
    description:
      'Please use the `balance` field in the `ranger.yaml` config file instead.\nMore info: https://ranger.tcbinternal.net/docs/configuration/ranger_yaml/object_structure/balance#overview',
  },
  {
    key: 'START_BALANCE',
    description:
...
function checkPasswordReset() {
  ...
  if (!emailEnabled && passwordResetAllowed) {
    logger.warn(
      ...
      https://ranger.tcbinternal.net/docs/configuration/authentication/email
...
export function checkVariables() {
  ...
  if (hasDefaultSecrets) {
    ...
    https://ranger.tcbinternal.net/toolkit/creds_generator
...
Latest version: ${Constants.CONFIG_VERSION}
      ...
      https://ranger.tcbinternal.net/changelog\n\n`,
    );
  }
}
    ...
    if (typeof value === 'string') {
      ...
      if (varName) {
        ...
        logger.warn(
          ...
          This is incorrect! You should use environment variable references in your ranger.yaml file, such as:
