/**
 * Date and time functions for Excel formula parser
 * Adapted from fast-formula-parser
 */
...
regFunc('DATEVALUE', (dateText: EvalResult) => {
  const parsedDate = parseDate(dateText as string);
  const {date, isDateGiven} = parseDateWithExtra(parsedDate);
  if (!isDateGiven) {
    return 0;
  }
...
export function DAYS(endDate: EvalResult, startDate: EvalResult) {
  const endDateTime = parseDate(endDate as string).getTime();
  const startDateTime = parseDate(startDate as string).getTime();
  let offset = 0;
  if (startDateTime < -2203891200000 && -2203891200000 < endDateTime) {
    offset = 1;
  }
  return Math.floor(endDateTime - startDateTime) / MS_PER_DAY + offset;
}
...
export function DAYS360(start_date: Date, end_date: Date, method: boolean) {
  ...
  if (method) {
    ...
    // If start date is last day of month, set to 30
    const startDay = start_date.getDate();
    sd = startDay === smd ? 30 : startDay;
    
    // Handle end date adjustment
    const endDay = end_date.getDate();
    if (endDay === 31) {
      // End date is 31st of month
      if (sd < 30) {
        // Start date was earlier than 30th, roll to next month day 1
        em++;
        ed = 1;
      } else {
        // Start date was 30th or 31st (sd=30), set end date to 30
        ed = 30;
      }
    } else {
      // End date is not 31st, use as-is
      ed = endDay;
    }
  }

  const result = 360 * (end_date.getFullYear() - start_date.getFullYear()) +
    30 * (em - sm) +
    (ed - sd);
...
regFunc(
  ...
  (startDate: EvalResult, endDate: EvalResult, method: EvalResult) => {
    ...
    const parsedMethod = getBoolean(method, false)!;

    return DAYS360(parsedStartDate, parsedEndDate, parsedMethod);
  }
);

regFunc('EDATE', (startDate: EvalResult, months: EvalResult) => {
  const parsedStartDate = parseDate(startDate as string);
  const parsedMonths = getNumberOrThrow(months);
  parsedStartDate.setUTCMonth(parsedStartDate.getUTCMonth() + parsedMonths);
  return toSerial(parsedStartDate);
});

regFunc('EOMONTH', (startDate: EvalResult, months: EvalResult) => {
  const parsedStartDate = parseDate(startDate as string);
  const parsedMonths = getNumberOrThrow(months);
  parsedStartDate.setUTCMonth(parsedStartDate.getUTCMonth() + parsedMonths + 1, 0);
  return toSerial(parsedStartDate);
});
...
regFunc(
  ...
  (startDate: EvalResult, endDate: EvalResult, holidays: EvalResult) => {
    let parsedStartDate = parseDate(startDate as string);
    let parsedEndDate = parseDate(endDate as string);
    let sign = 1;
    ...
    while (parsedStartDate <= parsedEndDate) {
      ...
      if (parsedStartDate.getUTCDay() !== 0 && parsedStartDate.getUTCDay() !== 6) {
        let found = false;
        for (let i = 0; i < holidaysArr.length; i++) {
          if (compareDateIgnoreTime(parsedStartDate, holidaysArr[i])) {
            found = true;
            break;
          }
        }
...
regFunc(
  ...
  (
    ...
    let parsedWeekend: number[] | undefined;
    ...
    if (typeof weekend === 'string' && Number(weekend).toString() !== weekend) {
      // Validate weekend string is exactly 7 characters
      if (weekend.length !== 7) throw FormulaError.VALUE;
      // Validate weekend string contains only 0s and 1s
      if (!/^[01]{7}$/.test(weekend)) throw FormulaError.VALUE;
      const weekendString = weekend.charAt(6) + weekend.slice(0, 6);
      const weekendArr: number[] = [];
      for (let i = 0; i < weekendString.length; i++) {
        if (weekendString.charAt(i) === '1') weekendArr.push(i);
      }
      parsedWeekend = weekendArr;
    } else {
      // using weekend number (or string that converts to valid number like "1")
      const weekendNumber = typeof weekend === 'number' ? weekend : Number(weekend);
      // If it's a string that looks like a binary weekend (all 0s and 1s), validate length
      if (typeof weekend === 'string' && /^[01]+$/.test(weekend)) {
        // String of 0s and 1s but not exactly 7 chars = invalid
        if (weekend.length !== 7) throw FormulaError.VALUE;
      }
      // Validate weekend number is valid (1-17)
      if (!WEEKEND_TYPES[weekendNumber]) throw FormulaError.VALUE;
      parsedWeekend = WEEKEND_TYPES[weekendNumber];
    }
    ...
    while (parsedStartDate <= parsedEndDate) {
      let skip = false;
      if (Array.isArray(parsedWeekend)) {
        for (let i = 0; i < parsedWeekend.length; i++) {
          if (parsedWeekend[i] === parsedStartDate.getUTCDay()) {
            skip = true;
            break;
          }
        }
...
regFunc('TIMEVALUE', (timeText: EvalResult) => {
  const parsedTime = parseDate(timeText as string);
  return (
    (3600 * parsedTime.getUTCHours() +
      60 * parsedTime.getUTCMinutes() +
...
function WORKDAY_INTL(
  ...
  const parsedStartDate = parseDate(startDate as string);
  const parsedDays = getNumberOrThrow(days);
  ...
  return toSerial(parsedStartDate) - 1;
}
...
regFunc(
  ...
  (startDate: EvalResult, endDate: EvalResult, basis: EvalResult) => {
    ...
    const parsedBasis = Math.trunc(getNumberWithDefault(basis, 0));

    return YEARFRAC(parsedStartDate, parsedEndDate, parsedBasis);
  }
);
