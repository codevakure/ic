...
export function createUserGroupMethods(mongoose: typeof import('mongoose')) {
  ...
  async function ensureDefaultGroups(): Promise<boolean> {
    ...
    const defaultGroups = [
      {
        name: 'LibreChatConsumer',
        description: 'Default consumer group for LibreChat platform',
        source: 'local' as const,
        memberIds: [],
      },
      {
        name: 'LibreChatEconomic',
        description: 'Default economic group for LibreChat platform',
        source: 'local' as const,
        memberIds: [],
      },
      {
