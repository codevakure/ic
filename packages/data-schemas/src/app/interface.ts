import { removeNullishValues } from 'ranger-data-provider';
import type { TCustomConfig, TConfigDefaults } from 'ranger-data-provider';
...
export async function loadDefaultInterface({
  ...
  const loadedInterface: AppConfig['interfaceConfig'] = removeNullishValues({
    ...
    customLogo: interfaceConfig?.customLogo ?? defaults.customLogo,
    themeColors: interfaceConfig?.themeColors ?? defaults.themeColors,
