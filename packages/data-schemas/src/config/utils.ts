 ...
 * Priority:
 * 1. RANGER_LOG_DIR environment variable
 * 2. If running within Ranger monorepo (when cwd ends with /api), use api/logs
...
export const getLogDirectory = (): string => {
  if (process.env.RANGER_LOG_DIR) {
    return process.env.RANGER_LOG_DIR;
  }
  ...
  // For Ranger project structure, use api/logs
  // For external consumers, they should set RANGER_LOG_DIR
  if (cwd.includes('Ranger')) {
    return apiLogsPath;
  }
